...
class PN {
  ...
  getPushMethodByRegId(regId) {
    ...
    if (this.settings.isAlwaysUseFCM) {
      const pushMethod = this.settings.useFCMMethodInsteadOfGCM ? _constants.FCM_METHOD : _constants.GCM_METHOD;
      return {
        regId,
        pushMethod
      };
  ...
  send(_regIds, data, callback) {
    ...
    regIds.forEach(regIdOriginal => {
      ...
      } else if (pushMethod === _constants.GCM_METHOD || pushMethod === _constants.FCM_METHOD) {
    ...
    try {
      ...
      if (regIdsFCM.length > 0) {
        const method = this.settings.useFCMMethodInsteadOfGCM ? _sendFCM.default : _sendGCM.default;
