...
const _require = require('./constants'),
      FCM_METHOD = _require.FCM_METHOD;
...
const getRecipientList = R.cond([[R.has('tokens'), R.prop('tokens')], [R.has('token'), propValueToSingletonArray('token')], [R.has('condition'), propValueToSingletonArray('condition')], [R.has('topic'), propValueToSingletonArray('topic')]]);
...
const sendChunk = (firebaseApp, recipients, message) => {
  ...
  return firebaseAdmin.messaging(firebaseApp).sendEachForMulticast(firebaseMessage).then((response, err) => {
...
const sendFCM = (regIds, data, settings) => {
  ...
  const opts = {
    credential: settings.fcm.credential || firebaseAdmin.credential.cert(settings.fcm.serviceAccountKey)
  };
  ...
  if (containsValidRecipients(data)) {
    ...
    do {
      ...
      promises.push(sendChunk(firebaseApp, {
